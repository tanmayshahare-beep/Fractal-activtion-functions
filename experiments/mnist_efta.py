"""
CNN with Exponential Fractal Tree Activation (EFTA) on MNIST - PyTorch

This script implements a Convolutional Neural Network using EFTA as the activation function
on the standard MNIST handwritten digit dataset.
...
def plot_results(results, project_root=None):
    ...
    ax.set_title('MNIST - Final Test Accuracy')
    ...
    if project_root:
        ...
        plt.savefig(os.path.join(outputs_dir, 'mnist_efta_comparison.png'), dpi=150, bbox_inches='tight')
        print(f"\nPlots saved to: {os.path.join(outputs_dir, 'mnist_efta_comparison.png')}")
    else:
        plt.savefig('mnist_efta_comparison.png', dpi=150, bbox_inches='tight')
    plt.show()
...
def print_comparison_table(results):
    ...
    print("\n" + "="*90)
    print("MNIST CNN ACTIVATION COMPARISON RESULTS".center(90))
    print("="*90)
...
def main():
    print("="*70)
    print("CNN with EFTA on MNIST Handwritten Digits (PyTorch)")
    print("="*70)
    ...
    os.makedirs(outputs_dir, exist_ok=True)
    np.save(os.path.join(outputs_dir, 'mnist_efta_results.npy'), results, allow_pickle=True)
    print(f"\nResults saved to: {os.path.join(outputs_dir, 'mnist_efta_results.npy')}")

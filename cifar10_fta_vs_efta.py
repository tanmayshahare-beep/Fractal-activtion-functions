"""
FTA vs EFTA Comparison on CIFAR-10 - Memory Optimized

Optimized for single GPU with memory management to prevent overheating.
- Reduced batch size
...
def setup_gpu():
    ...
    if torch.cuda.is_available():
        ...
        torch.cuda.set_per_process_memory_fraction(0.7, 0)  # Use max 70% of GPU memory
        
        print(f"\n{'='*70}")
        print(f"GPU: {torch.cuda.get_device_name(0)}")
        print(f"Total Memory: {torch.cuda.get_device_properties(0).total_memory / 1e9:.2f} GB")
        print(f"Memory Limit: 70% ({torch.cuda.get_device_properties(0).total_memory * 0.7 / 1e9:.2f} GB)")
        print(f"{'='*70}\n")
        return device
    else:
...
# =============================================================================
# CNN Architecture for CIFAR-10 (4-Block)
# =============================================================================

class CNNFTA(nn.Module):
    """4-Block CNN with Fractal Tree Activation for CIFAR-10."""
...
class CNNEFTA(nn.Module):
    """4-Block CNN with Exponential Fractal Tree Activation for CIFAR-10."""
    def __init__(self, depth=2, branch_factor=2):
        super().__init__()
        self.depth = depth
        self.branch_factor = branch_factor
...
def create_model(model_type, depth, branch_factor, device):
    """Create model based on type."""
    if model_type == 'fta':
        return CNNFTA(depth=depth, branch_factor=branch_factor).to(device)
    elif model_type == 'efta':
        return CNNEFTA(depth=depth, branch_factor=branch_factor).to(device)
    else:
...
def get_data_loaders(batch_size=128, val_split=0.1):
    """Get train/val/test data loaders for CIFAR-10."""
    ...
    # Reduced num_workers to prevent memory issues
...
def run_experiment(epochs=30, verbose=True):
    """Run FTA vs EFTA comparison experiment."""
    print(f"\n{'='*70}")
    print("FTA VS EFTA COMPARISON ON CIFAR-10 (Memory Optimized)")
    print(f"{'='*70}\n")

    # Get data loaders
    train_loader, val_loader, test_loader = get_data_loaders()

    # Model configurations (reduced set for faster training)
    configurations = [
        # FTA variants
        ('fta', 2, 2),
        ('fta', 3, 2),
    ...
    for idx, (model_type, depth, branch_factor) in enumerate(configurations):
        ...
        try:
            model = create_model(model_type, depth, branch_factor, device)
...
def print_results_table(results):
    ...
    print(f"\n{'='*80}")
    print("CIFAR-10 RESULTS: FTA VS EFTA")
    print(f"{'='*80}")
...
def save_results(results, output_dir='./outputs/results'):
    ...
    filename = f'cifar10_fta_vs_efta_{timestamp}.json'
...
def main():
    ...
    print(f"Architecture: 4-block CNN (64→128→256→512)")
    ...
    # Run experiment
    results = run_experiment(epochs=30, verbose=True)

...
SPECIFICALLY FOR: EFTA and REAct-EFTA.

This script is a self-contained version of the reviewer experiments, including
only the most complex activation layers (EFTA, REAct-EFTA) inlined.

Usage:
    python review_efta_react_standalone.py
"""
...
class ActivationWrapper(nn.Module):
    def __init__(self, activation_layer):
        ...
        self.is_fta_family = isinstance(activation_layer, (ExponentialFTA, ReActEFTA))
...
def get_activation(act_type, num_channels):
    if act_type == 'efta':
        ...
        # Relu fallback for initial conv
...
class ResNetCustom(nn.Module):
    def __init__(self, block, layers, num_classes=10, act_type='efta'):
    ...
    def _make_layer(self, block, planes, blocks, stride=1, act_type='efta'):
        downsample = None
...
def resnet18_custom(act_type='efta', num_classes=10):
...
def setup_device():
    if torch.cuda.is_available():
        # Allowing up to 90% of memory to ensure it's not capped at 6-7GB
        # On an 80GB card, this is ~72GB.
...
def train_model(model, trainloader, testloader, device, epochs=50, act_name='efta'):
...
def run():
    ...
    with open(f'outputs/results/review_efta_react_standalone_{timestamp}.json', 'w') as f:

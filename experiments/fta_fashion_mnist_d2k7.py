"""
FTA (d=2, k=7) on Fashion-MNIST - PyTorch

This script trains an FTA model with:
- depth=2, branch_factor=7
- Total leaves: 7^2 = 49 leaves
...
Usage:
    python experiments/fta_fashion_mnist_d2k7.py
"""
...
class CNNFTA(nn.Module):
    ...
    def __init__(self, depth=2, branch_factor=7, input_channels=1, num_classes=10):
        super().__init__()
    ...
    def forward(self, x):
        ...
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.fta1(x.permute(0, 2, 3, 1)).permute(0, 3, 1, 2)
...
def train_model(model, train_loader, val_loader, test_loader, device,
                epochs=50, model_name="FTA (d=2, k=7)", use_amp=True):
...
def print_results(result):
    ...
    print("\n" + "="*70)
    print("FTA (d=2, k=7) FASHION-MNIST RESULTS".center(70))
    print("="*70)
    print(f"Model: {result['name']}")
    print(f"Depth: 2, Branch Factor: 7")
    print(f"Total Leaves: 7^2 = 49")
    ...
    print(f"  FTA (d=2,k=7): {result['test_acc']:.2f}% (this work)")

    if result['test_acc'] > 92.5:
        print("\n✅ FTA (d=2,k=7) OUTPERFORMS all smaller configurations!")
    elif result['test_acc'] > 92.0:
        print("\n✅ FTA (d=2,k=7) shows strong performance!")
    elif result['test_acc'] > 91.8:
        print("\n✅ FTA (d=2,k=7) outperforms FTA (d=2,k=2)!")
    elif result['test_acc'] > 91.2:
        print("\n✅ FTA (d=2,k=7) outperforms Maxout baseline!")
    elif result['test_acc'] > 90.5:
        print("\n✅ FTA (d=2,k=7) outperforms ReLU baseline!")
...
def main():
    print("="*70)
    print("FTA (d=2, k=7) on Fashion-MNIST (PyTorch)")
    print("="*70)
    print("\nConfiguration:")
    print("  Depth: 2")
    print("  Branch Factor: 7")
    print("  Total Leaves: 7^2 = 49")
    ...
    # Create model
    print("\nCreating FTA model (d=2, k=7)...")
    model = CNNFTA(depth=2, branch_factor=7)
    ...
    plot_path = os.path.join(outputs_dir, 'fta_d2k7_fashion_mnist_training.png')
    plot_training_history(result, save_path=plot_path)

    # Save results
    results_dir = os.path.join(project_root, 'outputs', 'results')
    os.makedirs(results_dir, exist_ok=True)
    results_path = os.path.join(results_dir, 'fta_d2k7_fashion_mnist_results.npy')

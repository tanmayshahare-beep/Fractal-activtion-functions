"""
FTA vs EFTA Comparison on CIFAR-10 - CUDA Parallel Training

Uses your NVIDIA RTX 4060 with parallel training via multiple CUDA streams.
Different model configurations train simultaneously on the same GPU.
"""
...
def train_model(model_type, depth, branch_factor, train_loader, val_loader, test_loader,
                device, epochs=50, stream=None, result_queue=None):
    """Train a model."""
    
    model_name = get_model_name(model_type, depth, branch_factor)
    print(f"\n[Stream] Starting: {model_name}")
    ...
    try:
        ...
        for epoch in range(epochs):
            ...
            if (epoch + 1) % 10 == 0:
                print(f"[Stream] {model_name} - Epoch {epoch+1:2d}/{epochs}: "
                      f"Train Acc={train_acc:.2f}%, Val Acc={val_acc:.2f}%")

            if patience_counter >= early_stop_patience:
                print(f"[Stream] {model_name} - Early stopping at epoch {epoch+1}")
                break
        ...
        print(f"[Stream] ✓ {model_name} completed: Test Acc={test_acc:.2f}%, Time={total_time/60:.1f}min")
        ...
        if result_queue:
            result_queue.put({'name': model_name, 'error': str(e)})
...
# =============================================================================
# Parallel Training on Single GPU
# =============================================================================

def run_parallel_training(device, epochs=50):
    """Run training in parallel using multiple threads on same GPU."""
    
    print(f"\n{'='*70}")
    print("PARALLEL TRAINING ON SINGLE GPU")
    print(f"{'='*70}")
    ...
    print(f"\nTraining {len(configurations)} models in parallel on {torch.cuda.get_device_name(device.index)}")
    print(f"{'='*70}\n")
    ...
    for config in configurations:
        ...
        thread = threading.Thread(
            target=train_model,
            args=(
                model_type, depth, branch_factor,
                train_loader, val_loader, test_loader,
                device, epochs, None, result_queue
            ),
            daemon=True
        )
    ...
    print(f"Launched {len(threads)} parallel training jobs\n")
...
def print_results_table(results, total_time):
    ...
    print(f"\n{'='*100}")
    print("CIFAR-10 RESULTS: FTA VS EFTA (Parallel Training)")
    print(f"{'='*100}")
    print(f"{'Model':<25} | {'Test Acc':<10} | {'Val Acc':<10} | {'Params':<12} | {'Time (min)':<10}")
    print("-"*100)
...
def save_results(results, total_time, output_dir='./outputs/results'):
    ...
    filename = f'cifar10_fta_vs_efta_parallel_{timestamp}.json'
...
def main():
    """Main entry point."""
    print(f"\n{'='*70}")
    print("FTA VS EFTA ON CIFAR-10 - PARALLEL TRAINING")
    print(f"{'='*70}")
    ...
    if not torch.cuda.is_available():
        print("\n⚠ Warning: CUDA not available. Parallel training benefits limited.")
    
    # Run parallel training
    results, total_time = run_parallel_training(device, epochs=50)
